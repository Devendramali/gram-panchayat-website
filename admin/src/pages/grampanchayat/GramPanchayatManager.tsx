...
export default function GramPanchayatMembers() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Validation: Only one Sarpanch/Upasarpanch
      if (!editId) {
        if (
      ...
      data.append("role", formData.role === "Custom" ? formData.customRole : formData.role);
      ...
      fetchMembers();
      setFormData({ name: "", role: "Sarpanch", customRole: "", contact: "", image: null });
      setEditId(null);
      setShowModal(false);
    } catch (err) {
  ...
  const handleEdit = (member) => {
    setFormData({
      name: member.name,
      role: ["Sarpanch", "Upasarpanch", "Member"].includes(member.role) ? member.role : "Custom",
      customRole: ["Sarpanch", "Upasarpanch", "Member"].includes(member.role) ? "" : member.role,
...
const handleDelete = async (id) => {
  ...
  try {
    await API.delete(`/members/${id}`); // <-- now works
          ...
          <div className="rounded-2xl border border-gray-200 bg-white">
            ...
            <div className="max-w-full overflow-x-auto">
              <div className="min-w-[900px]">
                ...
                {members.length === 0 && (
                  <p className="text-center py-6 text-gray-400">No members added yet</p>
                )}
              </div>
            </div>
          </div>
